...
CREATE TABLE `ConfigRequest` (
    ...
    `lvl_1_aprrover` VARCHAR(191) NOT NULL,
    `lvl_2_aprrover` VARCHAR(191) NULL,
    `lvl_3_aprrover` VARCHAR(191) NULL,
...
CREATE TABLE `roles` (
    ...
    `user_modified` VARCHAR(36) NOT NULL,
    `date_created` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `date_modified` DATETIME(3) NULL,
...
CREATE TABLE `transactions` (
    ...
    `transport_vehicle_plate_no` VARCHAR(10) NULL,
...
-- AddForeignKey
ALTER TABLE `_ConfigToSite` ADD CONSTRAINT `_ConfigToSite_A_fkey` FOREIGN KEY (`A`) REFERENCES `configuration`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_ConfigToSite` ADD CONSTRAINT `_ConfigToSite_B_fkey` FOREIGN KEY (`B`) REFERENCES `sites`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
